...
get_signed_url() {
  ...
  local url="$server/v1/libs/noise-canceller?lib_version=$version&platform=js-sdk"
  g_signed_url=$(
...
main() {
  ...
  if $need_install; then
    ...
    local commands=("pnpm add" "yarn add" "npm install")
    local install_command=""

    for i in "${!lock_files[@]}"; do
      if [ -f "${lock_files[$i]}" ]; then
        install_command="${commands[$i]}"
        break
      fi
    done

    if [ -z "$install_command" ]; then
      println_message "Default: Using npm for installation."
      install_command="npm install"
    fi

    println_message "Executing: $install_command $g_download_file_path"
    eval "$install_command $g_download_file_path"
  fi
}
