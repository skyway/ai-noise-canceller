...
get_signed_url() {
  ...
  local url="$server/v1/libs/noise-canceller?lib_version=$version&platform=android-sdk"
  g_signed_url=$(
...
usage() {
  ...
  println_message "  --download-only       Do not install, only download"
  ...
  println_message "  $0 --download-only --version=1.0.0 --dest=/path/to/dir"
...
main() {
  ...
  if $need_install; then
    if [[ "$g_download_file_path" =~ skyway-ai-noise-canceller-([0-9]+\.[0-9]+\.[0-9]+)\.zip ]]; then
      local download_file_version="${BASH_REMATCH[1]}"
    else
      abort "No version is specified in $g_download_file_path"
    fi

    unzip $g_download_file_path
    local install_command="mvn install:install-file "
    local unziped_file_dir="com/ntt/skyway/audio/ai-noise-canceller/$download_file_version"
    local aar_file_path="$unziped_file_dir/ai-noise-canceller-$download_file_version.aar"
    local pom_file_path="$unziped_file_dir/ai-noise-canceller-$download_file_version.pom"

    println_message "Executing: $install_command -Dfile=$aar_file_path -DpomFile=$pom_file_path"
    eval "$install_command -Dfile=$aar_file_path -DpomFile=$pom_file_path"
    print_message "Remove unziped file..."
    rm -rf "com"
    println_message "Completed!!"
  fi
}
